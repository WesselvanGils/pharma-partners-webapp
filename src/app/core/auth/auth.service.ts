...
export class AuthService
{
    ...
    // Sends a register request to the backend api taking the userdata as a User object as input
    // Saves the user to local storage if the register is successfull
    // Returns an error if the register was unsuccessfull or an error occured at the api
	register(userData: User): Observable<User>
	{
		this.loggingService.log(`User with data: ${userData} attempted to register.`)

		return this.http
			.post<User>(`${environment.apiUrl}auth/signup`, userData, {
				headers: this.headers,
			})
			.pipe(
				map((user) =>
				{
                    // Executes if the register was successfull
                    this.saveUserToLocalStorage(user);
					this.currentUser$.next(user);
					this.alertService.success('You have been signed up!');
					return user;
				}),
				catchError((error: any) =>
				{
                    // Executes if the register was unsuccessfull or if an error occured
					console.log('error:', error);
					console.log('error.message:', error.message);
					console.log('error.error.message:', error.error.message);
					this.alertService.error(error.error.message || error.message);
